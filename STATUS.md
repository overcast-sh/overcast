...
| S3 Tables       | 49  | Table buckets, namespaces, Iceberg tables with a real `--table-s3` warehouse bucket each, `versionToken` commits, and the Iceberg REST catalog at `/iceberg` (commits, staged create); each bucket a Glue `s3tablescatalog` catalog that Athena queries and writes; maintenance/replication stored, never run |
