...
### The .NET emitter, and the SDK type table
...
Three measured facts about AWSSDK v4 keep the spelling small, and the
emitter's own header records how each was measured:
...
What they do not settle is a member AWSSDK customizes away from its modeled
kind. `AWSSDK.CloudWatchLogs` types `InputLogEvent.Timestamp` as `DateTime?`
where the model says `long`, so a `long` written from the model fails the
suite's build with `CS0029`. So the .NET emitter resolves each member's AWSSDK
type at emit time, as the Go emitter does (#1831) — but it cannot load an
assembly: this program runs in CI's docs job, which has no .NET SDK and no
NuGet cache, and `-check` must stay offline and byte-reproducible. It reads a
**committed table** instead.

| piece | what it is |
| --- | --- |
| `compat/suites/dotnet-sdk/sdk-types/AWSSDK.<Service>.txt` | every model class of one pinned package and each property's type, in a closed grammar (`long?`, `DateTime?`, `enum:<Name>`, `class:<Name>`, `List<T>`, …), headed by the package id and version. Generated by the suite itself: `Scenario/SdkTypeTable.cs` reflects over the assemblies it compiles against |
| the refresh command | `docker build -f compat/suites/dotnet-sdk/Dockerfile --target sdk-types --output type=local,dest=compat/suites/dotnet-sdk/sdk-types compat/suites`. It needs Docker, not a .NET SDK, and is a target of its own ahead of the image's test stage, so refreshing a stale table never needs the check it fixes to pass first |
| `dotnetsdktypes.go` | reads the table, and refuses — in generation and `-check` alike — a table whose package versions differ from `OvercastCompat.csproj`'s pins, naming the refresh command |
| `SdkTypeTableTests` (the suite's) | renders the table from the pinned assemblies and fails on any difference from the committed files, in the image build and in `test.yml`'s compat-suite-unit-tests job |

A digest of the files in the style of `models/aws/VERSION` would add nothing
to the last row: it proves a file is what some run wrote, where the suite's
test proves it is what the pinned assemblies declare.

Where the model and the table agree, the spelling is the one this emitter has
always written, down to the byte; the SDK's type decides the scalar a literal
is range-checked against and `Bind<T>` is instantiated with. Where they
disagree, one pair is spelled and the rest are refused:

| model | AWSSDK | spelled as |
| --- | --- | --- |
| `long` | `DateTime?`, in a service listed in `dotnetEpochMilliseconds` | a literal `System.DateTimeOffset.FromUnixTimeMilliseconds(nL).UtcDateTime`; an expression `b.EpochMilliseconds(…)` |
| anything else | a type the modeled kind does not convert to | refused, `dotnet-emit-unsupported:<Member>` |

`dotnetEpochMilliseconds` is a table of measurements rather than a rule: the
type table can say a property is a `DateTime` where the model says `long`, but
not what the SDK sends for it, and a customization that meant seconds would
compile just as well. CloudWatch Logs is the one row, measured both ways by the
suite's `SdkWireFormTests`.

The same disagreement matters on the way back. The suite reads a response as
a document, and a `DateTime` renders there as ISO text, where every other
backend holds the number the service sent — so an `equals`, a `where` or an
exported `$ref` would disagree. For every request and response class a group's
calls touch, the emitter registers each such property with
`Documents.EpochMilliseconds` in the emitted constructor, which renders it as
that number; a response property it could not render alike refuses the group.

The refusals, all of which scope the group away from `dotnet-sdk` in the
registry:

| refusal | what it means |
| --- | --- |
| the modeled kind has no C# literal | a timestamp, document, union, bigInteger or bigDecimal |
...
| the pinned package lacks the operation or the member | recorded as `:<Op>Request`, or under the member — a suite-wide compile error before the table |
| the model and AWSSDK disagree with no measured spelling | above |
| a request or response property whose document would differ | recorded under the property |
...
`testdata/golden/ScenariosWidgetsGen.cs.golden` is where it is reviewed, against
the stand-in table `testdata/dotnet-sdk-types/`. The proof that it *compiles*
is the dotnet-sdk suite's own Docker build.
