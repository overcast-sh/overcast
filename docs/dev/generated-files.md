...
| `internal/awsshapes/index.gen.go`, `internal/awsshapes/tables/*.txt` | committed — `make generate-aws-operations` | The Smithy shape tables Step Functions' `aws-sdk:` integrations serialise requests and read responses with, one text file per service in `models/aws/sdk-shapes-services.txt`. Cut by the same pruner as `models/aws/shapes/` from the same pinned checkout, so they are committed for the same reason as the manifest; `sdk-shapes-sha256` in `models/aws/VERSION` lets an ordinary pull request prove they are the generator's own bytes. Committed as text so a refresh reviews as a diff; not compiled in — see `internal/awsshapes/dist` below. |
...
| `compat/suites/dotnet-sdk/Groups/ScenariosGen.cs`, `Scenarios<Service>Gen.cs` | committed — `make generate-compat-model` | The C# the `dotnet-sdk` suite compiles for the generated groups: one file per service plus the index `Program.cs` registers as the suite's `ScenarioBackend`. Same split as go-sdk's — data plus typed calls, semantics hand-written in the suite's `Scenario/` — and, like go-sdk's, each member is spelled against the type the SDK gives it, read from the committed SDK type table below rather than from a loaded assembly. Emitted only while `scenarioBackends` in `cmd/compatgen/registry.go` names `dotnet-sdk` — the index is written either way, so the project always compiles. |
| `compat/suites/dotnet-sdk/sdk-types/AWSSDK.<Service>.txt` | committed — the dotnet-sdk suite's `sdk-types` Docker target (`docker build -f compat/suites/dotnet-sdk/Dockerfile --target sdk-types --output type=local,dest=compat/suites/dotnet-sdk/sdk-types compat/suites`) | The member types of every `AWSSDK.*` package `OvercastCompat.csproj` pins, reflected from the assemblies by the suite itself, one file per package. `cmd/compatgen` spells emitted C# from it without a .NET SDK, and refuses a table whose versions differ from the pins; the suite's `SdkTypeTableTests` fail when a file differs from the assemblies it names. Refresh it with every AWSSDK pin change. |
...
| `internal/awsshapes/dist/*.gz` | build output — `make aws-sdk-shapes` | The committed text tables, gzipped: what the binary embeds (~0.5 MB instead of ~4 MB). Same shape again: committed `.gitkeep`, packed files ignored; tests read the committed text via `OVERCAST_AWS_SDK_SHAPES_DIR`. |
...
| an `AWSSDK.*` pin in `compat/suites/dotnet-sdk/OvercastCompat.csproj` | the suite's `sdk-types` Docker target (the command is in [compat/suites/dotnet-sdk/README.md](../../compat/suites/dotnet-sdk/README.md#the-sdk-type-table)), then `make generate-compat-model` |
