...
func init() {
	capabilities.Default.Register(
		// DB instances
		capabilities.Capability{Service: "rds", Operation: "CreateDBInstance", Category: "DB instances", Status: capabilities.StatusSupported, Notes: "Docker-backed when available; async creating→available; mysql/postgres/mariadb/aurora-mysql/aurora-postgresql; master accounts can create databases/users and grant privileges; omitted `DBName` creates no MySQL/MariaDB database; Aurora members use cluster-owned placement, credentials, version, port, and database; omitted `PubliclyAccessible` defaults to false for Aurora and otherwise follows subnet placement; `ManageMasterUserPassword` generates a password and creates a Secrets Manager secret (`rds!db-<uuid>`), returned as `MasterUserSecret`; `Tags` applied via the shared tag store"},
		...
		// Aurora clusters
		capabilities.Capability{Service: "rds", Operation: "CreateDBCluster", Category: "Aurora clusters", Status: capabilities.StatusSupported, Notes: "aurora-mysql and aurora-postgresql only; logical cluster, Docker started on first instance; `ManageMasterUserPassword` generates a password and creates a Secrets Manager secret (`rds!cluster-<uuid>`), returned as `MasterUserSecret` and inherited by every member instance; `Tags` applied via the shared tag store"},
		...
		capabilities.Capability{Service: "rds", Operation: "ModifyDBCluster", Category: "Aurora clusters", Status: capabilities.StatusSupported, Notes: "`MasterUserPassword` applied to every member's engine; engine version, port and `DeletionProtection` applied; backup/maintenance windows, cluster parameter group, security groups and log exports recorded; `ManageMasterUserPassword` turns the managed-secret path on or off the same way ModifyDBInstance does"},
