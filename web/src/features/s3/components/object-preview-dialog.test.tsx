...
describe("ObjectPreviewDialog > CSV and TSV", () => {
  ...
  it("renders the rows in the grid, with the header and a row count", async () => {
    const size = serveObject(ORDERS_CSV)
    renderObject("orders.csv", { contentLength: size })
  ...
  it("right-aligns a numeric column", async () => {
    ...
    expect(cell.closest("[role=gridcell]")).toHaveClass("justify-end")
    expect(screen.getByText("Ada").closest("[role=gridcell]")).not.toHaveClass("justify-end")
  })
  ...
  it("toggles to the raw text and back, by keyboard, announcing which is on", async () => {
    ...
    renderObject("orders.csv", { contentLength: size })
    const user = userEvent.setup()
    const tableButton = await screen.findByRole("radio", { name: "Table" })
    ...
    expect(screen.queryByRole("grid")).not.toBeInTheDocument()
    expect(await screen.findByText(/id,name,amount/)).toBeInTheDocument()
  ...
  it("names a delimiter the text chose over the extension", async () => {
    const size = serveObject("name;price\nTea;1,50\n")
    renderObject("prices.csv", { contentLength: size })
  ...
  it("reads a TSV on its tabs", async () => {
    const size = serveObject("a\tb\n1,5\t2\n")
    renderObject("data.tsv", { contentLength: size })
    expect(await screen.findByText("1,5")).toBeInTheDocument()
  })

  it("offers the full-page viewer", async () => {
    const size = serveObject(ORDERS_CSV)
    renderObject("raw/orders.csv", { contentLength: size })
    const link = await screen.findByRole("link", { name: /Open in viewer/ })
    expect(link.getAttribute("href")).toBe("/s3/lake/view?key=raw%2Forders.csv")
  })

  it("closes on Escape", async () => {
    const size = serveObject(ORDERS_CSV)
    const onClose = vi.fn()
    renderObject("orders.csv", { contentLength: size, onClose })
...
describe("ObjectPreviewDialog > JSON Lines", () => {
  ...
  it("keeps records of different shapes as written", async () => {
    const text = '{"type":"click","x":1,"y":2}\n{"sku":"A","amount":3,"currency":"GBP"}\n'
    api.preview = { text, truncated: false }
    const size = serveObject(text)
    renderObject("events.ndjson", { contentLength: size })
  ...
  it("widens the dialog for a table, and only for a table", async () => {
    const size = serveObject('{"id":1}\n')
    const { unmount } = renderObject("users.jsonl", { contentLength: size })
    expect(await screen.findByRole("dialog")).toHaveClass("max-w-6xl")
    unmount()
    api.preview = { text: "notes", truncated: false }
...
describe("ObjectPreviewDialog > Parquet", () => {
  const fixture = (name: string) =>
    new Uint8Array(readFileSync(resolve(__dirname, "../../../lib/data-sources/__fixtures__", name)))

  it("shows the rows, their types and the file's own row count", async () => {
    const size = serveObject(fixture("orders.parquet"))
    renderObject("warehouse/orders/data/00000.parquet", { contentLength: size })
    const grid = await screen.findByRole("grid", { name: /Rows of/ })
    expect(await within(grid).findByText("Ada Lovelace")).toBeInTheDocument()
    expect(within(grid).getByText("STRING")).toBeInTheDocument()
    expect(screen.getByText("5 rows · 10 columns · 2 row groups · snappy")).toBeInTheDocument()
  })

  it("reads ZSTD, the codec Iceberg and S3 Tables write", async () => {
    const size = serveObject(fixture("orders.zstd.parquet"))
    renderObject("t.parquet", { contentLength: size })
  ...
  it("shows the schema on request", async () => {
    const size = serveObject(fixture("orders.parquet"))
    renderObject("t.parquet", { contentLength: size })
    await screen.findByRole("grid", { name: /Rows of/ })
    await userEvent.setup().click(screen.getByRole("radio", { name: "Schema" }))
    const schema = screen.getByRole("grid", { name: "Parquet schema" })
    expect(await within(schema).findByText("order_id")).toBeInTheDocument()
    expect(within(schema).getAllByText("required")).toHaveLength(2)
  ...
  it("explains a file that is not Parquet and offers the download instead", async () => {
    const size = serveObject("id,name\n1,Ada\n")
    renderObject("not-really.parquet", { contentLength: size })
